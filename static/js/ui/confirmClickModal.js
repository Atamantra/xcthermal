export function showClickConfirm(callback) {
  ...
  const handleYes = () => {
    ...
    // Add Meteoblue attribution if not present
    if (window.currentMap) {
      const attributionControl = window.currentMap._controls.find(c => c instanceof mapboxgl.AttributionControl);
      if (attributionControl) {
        const customAttribution = '<a href="https://www.meteoblue.com/" target="_blank">Meteoblue</a>';
        if (!attributionControl.options.customAttribution) {
          // If no custom attribution exists, we can try to re-add the control with our custom one
          // But modifying existing control options directly isn't officially supported in v2.
          // Easier way: Append to the attribution container DOM element directly if possible,
          // OR just add a new custom attribution string to the style source.
          // Method: Add a new attribution control if one doesn't exist with this string? No, duplicates.
          // Best Method for Mapbox GL JS v2: Update the style's implementation or append to DOM.

          // Let's go with direct DOM append for simplicity and robustness across versions, 
          // looking for the .mapboxgl-ctrl-attrib-inner container.
          const attribContainer = document.querySelector('.mapboxgl-ctrl-attrib-inner');
          if (attribContainer && !attribContainer.innerHTML.includes('Meteoblue')) {
            attribContainer.innerHTML += ' | ' + customAttribution;
          }
        } else if (Array.isArray(attributionControl.options.customAttribution)) {
          if (!attributionControl.options.customAttribution.includes(customAttribution)) {
            attributionControl.options.customAttribution.push(customAttribution);
            window.currentMap._updateAttribution();
          }
        } else if (typeof attributionControl.options.customAttribution === 'string') {
          if (!attributionControl.options.customAttribution.includes('Meteoblue')) {
            // It's a string, we can't easily push. Let's rely on the DOM method above which is safer.
            const attribContainer = document.querySelector('.mapboxgl-ctrl-attrib-inner');
            if (attribContainer && !attribContainer.innerHTML.includes('Meteoblue')) {
              attribContainer.innerHTML += ' | ' + customAttribution;
            }
        ...
        // No attribution control exists, add one
        window.currentMap.addControl(new mapboxgl.AttributionControl({
          customAttribution: '<a href="https://www.meteoblue.com/" target="_blank">Meteoblue</a>'
        }));
      }
    }
